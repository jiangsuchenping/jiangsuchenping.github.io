...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 初始化识字页面刷新按钮事件（只绑定一次）
    const refreshChineseBtn = document.getElementById('refresh-words-btn');
    if (refreshChineseBtn) {
        refreshChineseBtn.addEventListener('click', debounce(function() {
            // 禁用按钮，防止重复点击
            refreshChineseBtn.disabled = true;
            refreshChineseBtn.style.opacity = '0.5';
            ...
            // 500ms后重新启用按钮
            setTimeout(() => {
                refreshChineseBtn.disabled = false;
                refreshChineseBtn.style.opacity = '1';
            }, 500);
        }, 300));
    }
    
    // 初始化英语单词页面刷新按钮事件（只绑定一次）
    const refreshEnglishBtn = document.getElementById('refresh-english-btn');
    if (refreshEnglishBtn) {
        refreshEnglishBtn.addEventListener('click', debounce(function() {
            // 禁用按钮，防止重复点击
            refreshEnglishBtn.disabled = true;
            refreshEnglishBtn.style.opacity = '0.5';
            ...
            // 500ms后重新启用按钮
            setTimeout(() => {
                refreshEnglishBtn.disabled = false;
                refreshEnglishBtn.style.opacity = '1';
            }, 500);
        }, 300));
    }
...
function initChineseWordPage() {
    ...
    // 随机选择3个字进行展示
    const selectedWords = getRandomItems(chineseWords, 3);
...
function generateSmartMathProblem() {
    ...
    // 计算每个题目的权重
    const weightedProblems = allProblems.map(problem => ({
        ...problem,
        weight: calculateProblemWeight(mathProblemHistory[problem.problem])
    }));
...
function initMathTestPage() {
    ...
    window.checkAnswer = function(selectedAnswer) {
        ...
        if (isCorrect) {
            ...
            // 记录错题
            recordMathAnswer(currentProblem, false);
        }
...
function showHistoryDialog() {
    ...
    problems.forEach(record => {
        ...
        const hoursSinceLastAttempt = ((new Date() - new Date(record.lastAttemptTime)) / (1000 * 60 * 60)).toFixed(1);
        ...
        historyItem.innerHTML = `
            ...
            <div class="last-attempt">最后答题时间: ${lastAttempt} (${hoursSinceLastAttempt}小时前)</div>
            <div class="weight">权重分数: ${record.weight.toFixed(1)}</div>
        `;
