...
const currentLevelList = computed(() => {
  return hanziBank.value.filter(h => h.level <= currentLevel.value)
})
...
const levelProgress = computed(() => {
  const levelItems = hanziBank.value.filter(h => h.level === currentLevel.value)
...
onMounted(async () => {
  ...
  // Load Hanzi Bank dynamically
  const data = await import('../data/hanziBank.js')
  hanziBank.value = data.hanziBank
